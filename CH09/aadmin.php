...
<form method="post" action="api.php?do=adminmdy">
  <table width="100%">
    <tbody>
      ...
      <?php
      $rows = select("t10_admin", 1);
      foreach ($rows as $row) {
        ?>
        <tr>
          <td><input style="width:90%" type="text" name="acc[<?= $row['id'] ?>]" value="<?= $row['acc'] ?>"></td>
          <td><input style="width:90%" type="password" name="pwd[<?= $row['id'] ?>]" value="<?= $row['pwd'] ?>"></td>
