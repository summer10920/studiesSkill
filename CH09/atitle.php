...
<form method="post" action="api.php?do=titlemdy">
  <table width="100%">
    <tbody>
      ...
      <?php
      $rows = select("t3_title", 1);
      foreach ($rows as $row) {
        $rdo=($row['dpy'])?"checked":"";    //1 same true, 0 same false
        ?>
        <tr>
          ...
          <td>
            ...
            <input type="radio" name="radio" value="<?=$row['id']?>" <?=$rdo?>>
          </td>
          <td><input type="checkbox" name="del[]" value="<?=$row['id']?>"></td>
          <td><input type="button" onclick="op('#cover','#cvr','view.php?do=titlechg&id=<?=$row['id']?>')" value="更新圖片"></td>
        </tr>
      <?php
      }
      ?>
