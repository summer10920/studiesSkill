	...
	<div id="main">
		...
		<div id="ms">
			...
			<div class="di" style="height:540px; border:#999 1px solid; width:53.2%; margin:2px 0px 0px 0px; float:left; position:relative; left:20px;">
				...
				<script>
					var lin = new Array(); //必須要把輪播的資料已陣列方式放入到這變數
					lin=<?=json_encode($t5_ary)?>; //把PHP陣列以JSON方式編碼指定給JS的變數lin
					var now = 0;
					ww();
					if (lin.length > 1) setInterval("ww()", 3000);
					function ww() {
						$("#mwww").html("<embed loop=true src='" + lin[now] + "' style='width:99%; height:100%;'>")
						now++;
