...
<body>
	...
	<div id="main" style="overflow:scroll;border:0">
		<a title="<?= $t3_txt ?>" href="?">
...
<?=$t12txt?>
			...
			<div class="di di ad" style="height:540px; width:23%; padding:0px; margin-left:22px; float:left; ">
				...
				<div style="width:89%; height:480px;" class="dbor">
					...
					<script>
						var nowpage = 0,
							num = <?= $t6_num ?>; //要知道到底有多少圖

						function pp(x) {
							var s, t;
							if (x == 1 && nowpage - 1 >= 0) nowpage--; //按鈕上之事件
							if (x == 2 && nowpage + 1 <= (num - 3)) nowpage++; //按鈕下之事件
							/*當你有6張圖
							nowpage,display對象No1~No6
							0,123
							1,234
							2,345
							3,456 此時下按鈕失效了不給++
							*/
							$(".im").hide(); //先把所有圖做display:none;

							for (s = 0; s <= 2; s++) {
								t = s + nowpage;
								$("#ssaa" + t).show(); //把id="ssaa2" 做display;
								/*
								s,nowpage,{t}
								0,2,{2}
								1,2,{3}
